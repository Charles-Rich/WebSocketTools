...
internal class WebSocketConnection : BaseTcpSocketConnection<IWebSocketServer, IWebSocketConnection, string>, IWebSocketConnection
{
    #region 构造函数
    ...
    public async Task Send(string msgStr)
    {
        try
        {
            ...
            await Task.Run(() => { _serverEvent.OnSend?.Invoke(_server, this, msgStr); });
        }
        catch (Exception ex)
        {
